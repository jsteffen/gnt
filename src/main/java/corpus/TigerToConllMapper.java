    ...
    while ((line = reader.readLine()) != null) {
      if (startSentence && line.startsWith("#EOS")) {
        ...
        if (sentenceIndex > testSentIndex) {
          testWriter.newLine();
        } else if (sentenceIndex > develSentIndex) {
          develWriter.newLine();
        } else {
          trainWriter.newLine();
        }
      ...
      } else if ((!startSentence) && line.startsWith("#BOS")) {
        startSentence = true;
        tokenCnt = 0;
        sentenceIndex++;
      } else if (startSentence && (!line.startsWith("#5"))) {
        tokenCnt++;
